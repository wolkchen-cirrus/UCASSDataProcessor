...
import time, os, struct, json, fnmatch
...
parser.add_argument("--format", default=None, help="Change the output format between 'standard', 'json', and 'csv'. "
                                                   "For the CSV output, you must supply types that you want.")
parser.add_argument("--csv_sep", dest="csv_sep",
                    default=",", help="Select the delimiter between columns for the output CSV file. "
                                      "Use 'tab' to specify tabs. Only applies when --format=csv")
...
# Keep track of data from the current timestep. If the following timestep has the same data, it's stored in here too.
# Output should therefore have entirely unique timesteps.
while True:
